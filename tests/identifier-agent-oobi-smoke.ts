...
type AgentOobiCopyButtonState = 'missing' | 'loading' | 'success' | 'error' | 'ready';

const agentOobiCopyButtonStateInPage = (
    identifierAlias: string
): AgentOobiCopyButtonState => {
...
const waitForAgentOobiCopyButtonState = (
    page: Page,
    alias: string,
    expected: AgentOobiCopyButtonState,
    label: string,
    timeout = 30_000
): Promise<void> =>
    waitForCondition(
        async () =>
            (await page.evaluate(agentOobiCopyButtonStateInPage, alias)) ===
            expected,
        label,
        timeout
    );
...
const waitForAgentOobiCopySuccess = (
    ...
    waitForAgentOobiCopyButtonState(
        page,
        alias,
        'success',
...
const waitForAgentOobiCopyReady = (
    page: Page,
    alias: string
): Promise<void> =>
    waitForAgentOobiCopyButtonState(
...
try {
    ...
    const postCountBeforeFirstCopy = endRolePostUrls.length;
    logStage('identifier.copyAgentOobi.start', { alias });
    await clickVisibleTestId(page, `identifier-copy-agent-oobi-${alias}`);
    await waitForCondition(
        () => endRolePostUrls.length >= postCountBeforeFirstCopy + 1,
        'copy-agent-oobi end-role POST'
    );
    logStage('identifier.copyAgentOobi.posted', { alias });
    await sleep(500);
    if (endRoleFailureBodies.length > 0) {
        ...
        throw new Error(
            `Copy-agent-OOBI end-role POST failed: ${endRoleFailureBodies.join('\n')}\nverification=${verification}\n${endRolePostBodies.join('\n')}`
        );
    }
    if (endRolePostUrls.length !== postCountBeforeFirstCopy + 1) {
        throw new Error(
            `Expected one copy-agent-OOBI end-role POST, saw ${endRolePostUrls.length - postCountBeforeFirstCopy}: ${endRolePostBodies.join('\n')}`
        );
    }

    logStage('identifier.copyAgentOobi.oobiWait.start', { alias });
    const oobi = await waitForAgentEndRoleAndOobi({
        client,
        alias,
        agentPre,
    });
    logStage('identifier.copyAgentOobi.oobiWait.ready', { alias, oobi });
    await waitForAgentOobiCopySuccess(page, alias);
    logStage('identifier.copyAgentOobi.ready', { alias });

    const postCountBeforeSecondCopy = endRolePostUrls.length;
    logStage('identifier.copyAgentOobi.reuse.start', { alias });
    await waitForAgentOobiCopyReady(page, alias);
    await clickVisibleTestId(page, `identifier-copy-agent-oobi-${alias}`);
    await waitForAgentOobiCopySuccess(page, alias);
    logStage('identifier.copyAgentOobi.reuse.ready', { alias });
    if (endRolePostUrls.length !== postCountBeforeSecondCopy) {
        throw new Error(
